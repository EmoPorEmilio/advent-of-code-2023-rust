...
fn main() {
    ...
    for history in histories {
        ...
        let mut prev_first_digit = 0;
        println!("Differences: {:?}", differences);
        while i > 0 {
            let first_digit = differences[i][0];
            prev_first_digit = first_digit - prev_first_digit;
            i -= 1;
        }

        let first_digit = differences[0][0];
        prev_first_digit = first_digit - prev_first_digit;
        println!("Prev last digit: {:?}", prev_first_digit);
        sum_extrapolated_values += prev_first_digit;
